    ...
    public void openConnection() {
        ...
        try {
            ...
            if (!exists) {
                ...
                Set<String> colFamilySet = new HashSet<>();
                for (String col : columnNames) {
                    colFamilySet.add(col.split(":")[0]);
                }
                for (String colFamily : colFamilySet) {
...
//                }
    ...
    private byte[] intToBytes(Object column) {
        ...
        if (column instanceof Integer) {
            ...
            intValue = ((BigDecimal) column).intValue();
        } else {
    ...
    private byte[] longToBytes(Object column) {
        ...
        if (column instanceof Integer) {
            ...
            longValue = ((BigDecimal) column).longValue();
        } else {
    ...
    private byte[] doubleToBytes(Object column) {
        ...
        if (column instanceof Integer) {
            ...
            doubleValue = ((BigDecimal) column).doubleValue();
        } else {
    ...
    private byte[] floatToBytes(Object column) {
        ...
        if (column instanceof Integer) {
            ...
            floatValue = ((BigDecimal) column).floatValue();
        } else {
            throw new RuntimeException("Can't convert from " + column.getClass() + " to DOUBLE");
        }
    ...
    private byte[] shortToBytes(Object column) {
        ...
        if (column instanceof Integer) {
            ...
            shortValue = ((BigDecimal) column).shortValue();
        } else {
    ...
    private byte[] binaryToBytes(Object column) {
        byte[] bytes;
        if (column instanceof byte[]) {
            bytes = (byte[]) column;
        } else {
            throw new RuntimeException("Can't convert from " + column.getClass() + " to byte[]");
        }
        return bytes;
    }
