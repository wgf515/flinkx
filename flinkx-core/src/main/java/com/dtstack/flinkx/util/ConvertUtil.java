...
public final class ConvertUtil {
    ...
    public static RowTypeInfo buildRowTypeInfo(List<Map<String, String>> mapList) {
        String[] fieldTypes = mapList.stream().map(map -> map.get("type")).toArray(String[]::new);
        String[] fieldNames = mapList.stream().map(map -> map.get("name")).toArray(String[]::new);
        TypeInformation<?>[] typeInformation = ConvertUtil.toTypeInformations(fieldTypes);
        return new RowTypeInfo(typeInformation, fieldNames);
    }
